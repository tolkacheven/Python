...
sportsman_result = input('Введите результат спортсмена за первый день: ')
sportsman_target = input('Сколько должен составлять конечный результат спортсмена: ')
if sportsman_result.isdigit and sportsman_target:
    count_of_days = 1                               # Отсчет с 1, т.к. мы потратили день для замера первого результата
    sportsman_result = float(sportsman_result)
    sportsman_target = float(sportsman_target)
    while sportsman_result < sportsman_target:
        sportsman_result *= 1.1
        count_of_days += 1
    print(count_of_days)
